...
class RabbitConnection(object):
    ...
    def on_bindok(self, unused_frame):
        ...
        if self.consume_queue:
            ...
            LOGGER.info('Not consuming from Queue')
...
class Consumer(RabbitConnection):
    def __init__(self,amqp_url, io_loop):
       super(Consumer,self).__init__(amqp_url,io_loop,True)
...
def main():
    ...
    consumer = RabbitConnection('amqp://guest:guest@localhost:5672/%2F',io_loop,True,on_message=cb)
    producer = Producer('amqp://guest:guest@localhost:5672/%2F',io_loop)
  
    def send_data():
        producer.send_message("hello")
        
    periodic_callback = tornado.ioloop.PeriodicCallback(send_data, 1000,io_loop=io_loop)
    periodic_callback.start()
    
    #TODO application handler to change periodic callback based on config set
